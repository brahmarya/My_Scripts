...
lapply(1:5, function(m){
  ...
  lapply(1:nrow(data), function(n){
    train_data <- data[-n,]
    train <- train_data[!(duplicated(train_data[,4:(ncol(data) - 5)]) | duplicated(train_data[,4:(ncol(data)-5)], fromLast = TRUE)),]
    
    x <- as.matrix(train[,4:(ncol(data)-5)])
    y <- as.matrix(train[,(ncol(data)-4):ncol(data)])
    
    test_in <- as.matrix(data[n,4:(ncol(data)-5)])
  ...
  write.table(score,paste("E:/Academics/obj2-29March2019/Results/Hybrid/win_",win[m],"_predscore.tsv", sep = ""), sep = "\t");
  ...
  write.table(err, file = paste("E:/Academics/obj2-29March2019/Results/Hybrid/win_",win[m],"_err.tsv", sep = ""), sep = "\t")
})
