...
# VCF to fasta conversion -------------------------------------------------
   bg_cmd <- paste("~/anaconda3/bin/bgzip -f ",Path,"Rv_freebayes.vcf > ",Path,"Rv_freebayes.vcf.gz", sep = "")
   system(bg_cmd)
   system("PID=$!; wait $PID")
   
   tb_cmd <- paste("~/anaconda3/bin/tabix -f ",Path,"Rv_freebayes.vcf.gz > ",Path,"Rv_freebayes.vcf.gz.tbi", sep = "")
   system(tb_cmd)
   system("PID=$!; wait $PID")
   
   fst_cmd <- paste("bcftools consensus -f /storage/Collab/Mohit/Reference_genome/Mycobacterium_tuberculosis_H37Rv.fasta ",Path,"/Rv_freebayes.vcf.gz > /storage/Collab/Mohit/MDR_Fasta/", file[n], ".fasta", sep = "")
   system(fst_cmd)
})
