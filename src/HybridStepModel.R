...
lapply(1:nrow(data), function(n){
  train_data <- data[-n,]
  train <- train_data[!(duplicated(train_data[,2:35]) | duplicated(train_data[,2:35], fromLast = TRUE)),]
  
  x <- as.matrix(train[,2:35])
  y <- as.matrix(train[,(ncol(data)-4):ncol(data)])
  
  test_in <- as.matrix(data[n,2:35])
...
write.table(score,file = "E:/Academics/obj2-29March2019/Results/HybridStep/predscore.tsv", sep = "\t");
...
write.table(err, file = "E:/Academics/obj2-29March2019/Results/HybridStep/err.tsv", sep = "\t")
