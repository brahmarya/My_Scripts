...
files <- dir("E:/Academics/obj2-29March2019/RCSB/pat_pair/", full.names = TRUE);
data <- c();
...
data <- data[,c(1:35,(ncol(data)-4):ncol(data))]
data <- na.omit(data)
all_pred <- c()
lapply(1:nrow(data), function(n){
  train_data <- data[-n,]
  train <- train_data[!(duplicated(train_data[,4:35]) | duplicated(train_data[,4:35], fromLast = TRUE)),]
  
  x <- as.matrix(train[,4:35])
  y <- as.matrix(train[,(ncol(data)-4):ncol(data)])
  
  test_in <- as.matrix(data[n,4:35])
  ...
  write.table(score,file = "E:/Academics/obj2-29March2019/Results/HybridPair/predscore.tsv", sep = "\t");
  ...
  write.table(err, file = "E:/Academics/obj2-29March2019/Results/HybridPair/err.tsv", sep = "\t")
