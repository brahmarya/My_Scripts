...
# system("rm -rf /storage/obj2-29March2019/RCSB/pat")
...
fltr_static <- read.table("E:/Academics/obj2-29March2019/RCSB/dynaseq_feature_static.tsv")[,1:12]

fltr_ASA <- read.table("E:/Academics/obj2-29March2019/RCSB/all_dna.rsa")[,c(1,2,3,5,7,9,11,13)]

dna <-  sapply(rownames(fltr_static), function(x) strsplit(x,"[].[]")[[1]][2])

# system("rm -rf E:/Academics/obj2-29March2019/RCSB/pat")
...
mclapply(1:5, function(w){
  ...
  lapply(1:length(dna), function(n){
    ...
    sat <- fltr_static[str_detect(rownames(fltr_static), dna[n]),]
    ...
    lapply((frnk+1):(nrow(mat)-frnk), function(m){
      ...
      names(v2) <- rep(colnames(fltr_static),win_size[w])
    ...
    system(paste("mkdir -p E:/Academics/obj2-29March2019/RCSB/pat/win_",win, sep = ""))
    ...
    write.table(newMat, file = paste("E:/Academics/obj2-29March2019/RCSB/pat/win_",win,"/",naam[1],"_",naam[2],".tsv", sep = "", collapse = "_"), quote = FALSE, sep = "\t", row.names = FALSE )
  })
}, mc.cores = 1)
