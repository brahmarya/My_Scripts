...
lapply(1:4, function(m){
  data <- read.table(paste("E:/Academics/obj2-29March2019/RCSB/pat/win_",win[m],"_A.tsv", sep = ""), sep = "\t", header = TRUE)
  data <- data[,c(1:((win[m]*4)+3),(ncol(data)-4):ncol(data))]
  data <- na.omit(data)
  all_pred <- c()
  lapply(1:nrow(data), function(n){
    train_data <- data[-n,]
    train <- train_data[!(duplicated(train_data[,4:((win[m]*4)+3)]) | duplicated(train_data[,2:((win[m]*4)+1)], fromLast = TRUE)),]
    
    x <- as.matrix(train[,4:((win[m]*4)+3)])
    y <- as.matrix(train[,(ncol(data)-4):ncol(data)])
    
    test_in <- as.matrix(data[n,4:((win[m]*4)+3)])
  ...
  write.table(score,paste("E:/Academics/obj2-29March2019/Results/Seq/win_",win[m],"_predscore.tsv", sep = ""), sep = "\t");
  ...
  write.table(err, file = paste("E:/Academics/obj2-29March2019/Results/Seq/win_",win[m],"_err.tsv", sep = ""), sep = "\t")
})
